...
[![Tests](https://img.shields.io/badge/tests-35%20passing-8b5cf6)](app/decision.test.ts)
...
当前验证包含 35 个单元与集成测试、一组合成决策情境和一个时序 CNN 对照基准。离线评测不调用模型，也不读取真实用户资料。前端资源在启动时内嵌进服务进程，改动 `app/public/` 后需要重启开发服务器。
...
| 记忆库 | 结构化事实（类型/置信度/来源/生命周期）+ 事件聚合 + 截图去重 + 记忆中心（查/改/停用/删 + 检索理由） |
