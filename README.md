...
其中 $c_i$ 是观察置信度，$r_i$ 是来源可靠度，$\Delta t_i$ 是观察距今的天数；$\tau_s=21$ 天刻画短期，$\tau_l=240$ 天刻画长期。变化强度为：
...
只有有效样本量和差异同时越过门槛时，$\alpha_d$ 才增大：系统跟随持续的真实变化，同时滤除单次情绪波动。

### 2. 决策用途检索

证据 $e$ 对当前问题 $q$ 的分数由七路信号线性组合：

$$
S(e\mid q)=
\lambda_1 s_{\mathrm{sem}}+
\lambda_2 s_{\mathrm{lex}}+
\lambda_3 s_{\mathrm{time}}+
\lambda_4 s_{\mathrm{rel}}+
\lambda_5 s_{\mathrm{use}}+
\lambda_6 s_{\mathrm{contra}}-
\lambda_7 s_{\mathrm{dup}}
$$

$s_{\mathrm{contra}}$ 奖励能检验当前结论的反证，$s_{\mathrm{dup}}$ 惩罚重复内容。最终集合还受类别配额约束，避免十条证据都来自同一张截图。

### 3. 竞争假设与不确定性

状态假设 $h_k$ 使用 softmax 保留为分布，并计算归一化熵：

$$
p(h_k\mid E)=\frac{\exp z_k}{\sum_j\exp z_j},
\qquad
H=-\frac{1}{\log K}\sum_{k=1}^{K} p_k\log p_k
$$

其中 $K$ 是假设数量，因此 $H\in[0,1]$。总体不确定性由状态熵 $H$、证据冲突 $C$、模拟方差 $V$、证据覆盖 $G$ 和候选分差 $M$ 组成：

$$
U = 0.31H + 0.20C + 0.19V + 0.18(1-G) + 0.12(1-M)
$$

当 $U$ 高且证据覆盖 $G$ 低时，引擎优先追问或选择可逆策略。
...
其中 $\Delta t$ 是距上次更新的天数，对应 120 天的半衰期。最终策略效用综合目标契合 $R_g$、证据使用 $R_e$、一致性 $R_c$、自然度 $R_n$、信息价值 $I$ 与风险 $\mathcal R$，再加上学习先验和有界探索项：
...
探索项随情境样本量 $N_{c,a}$ 增长而收敛，探索强度随经验积累自动降低。
...
只有预期决策改善超过提问成本 $C_{\mathrm{ask}}(q)$ 时，系统才先补信息，用最少的一问区分最重要的两种解释。
