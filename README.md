...
![电子军师主界面：左侧档案、中间对话与可复制回复、顶栏思考深度与胆量滑杆、右侧判断面板](assets/app-home-v6.png)
...
<img src="assets/app-network-v6.png" align="right" width="340" alt="决策网络面板：分层激活图、最后选择、竞争解释与方案得分">
...
决策证据检索是三路混合：Okapi BM25、384 维特征哈希向量余弦、决策先验（时间、可靠度、重要性、历史用途），用 Reciprocal Rank Fusion 融合，再做嵌入空间去重、类别配额和反证覆盖。时间事实有有效区间；两条可靠信息互相冲突时，两条都会留下。
...
$\mu_d^{(\tau)}$ 是遗忘加权似然下的 MAP 位置估计；$\tau_s=21$ 与 $\tau_l=240$ 天给出两个互补估计器。变化门控是一个硬专家混合：仅当短期有效样本量 $N_{\mathrm{eff}}^{(s)}=\sum_i w_i^{(\tau_s)}$ 与偏移 $|\mu^{(\tau_s)}-\mu^{(\tau_l)}|$ 同时越过阈值时，短期权重才从 0.38 升到 0.72：
