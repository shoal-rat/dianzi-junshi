...
你可以在右侧展开“决策网络与完整思考过程”，看到：

- **决策网络图**：本轮真实计算的数据流（状态信念 → 体制假设 → 动作特征 → 预测状态 → 回应分布 → 输出），每个节点可点击查看激活值、参数和贡献权重——它画的是引擎真实执行的那次计算，可以逐个对账
- 它目前有多确定、哪些解释仍在竞争
- 参考了什么证据、比较过哪些方案、为什么没选其他方案
- 这次本地规划花了多久

顶栏有两个滑杆直接控制引擎：**思考深度**决定 rollout 推演深度与体制分支数；**胆量**重塑目标函数——风险惩罚、推进加成、追问成本和收手门槛都随它变化，急着推进就往右，想稳就往左。设置按聊天档案分别保存。
...
当前验证包含 20 个单元与集成测试，以及一组合成决策情境。离线评测不调用模型，也不读取真实用户资料。
...
| 决策控制 | 思考深度滑杆（rollout 深度 / 体制分支）+ 胆量滑杆（风险偏好重塑目标函数，按档案保存） |
...
| 前端 | 无框架 HTML / CSS / JavaScript + SVG 决策网络可视化 |

## 理论附录：它在优化什么

以下内容完整描述 v5.1–v5.2 决策引擎实现的数学结构。每个公式都对应仓库中的具体代码（主要在 `app/decision/worldmodel.ts`、`state.ts`、`evidence.ts`、`planner.ts`、`store.ts`），不是愿景描述。
...
风险直接由模型量给出——负面/失联概率加上压力越界的高斯尾概率（$\Phi$ 用 Abramowitz–Stegun erf 近似求值）：
...
同时推进类动作获得带符号的情境加成（$\delta=\beta-\tfrac12$，invite $+0.14\delta$、direct $+0.09\delta$、give\_space $-0.10\delta$），探索奖励缩放 $0.7+0.6\beta$，弃权门槛移动为 $0.76+0.14\delta\cdot 2$。谨慎档提前收手、多问一句；大胆档容忍更高方差、更快出手。同一世界模型，不同风险偏好，各自最优。
